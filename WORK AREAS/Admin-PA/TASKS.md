# Tasks

Standalone action items not tied to a full project session. Updated by Claude each session.

Format: `- [ ] Task description | Context | Due`

---

## 🔴 Urgent (due this week)

- [x] Complete editing rounds with Dad, Mark, and Emily on A-Team Workshop | RSG | Done 2026-06-01
- [x] Prep podcast episode briefs for batch filming | RSG | Done 2026-06-01
- [x] Get finance report from Laura | RSG | Done 2026-06-09
- [x] Email Brian Woods — we don't want to rent the offices any longer | Admin | Done 2026-06-09
- [x] Cancel Taki program and tell the team we'll just buy workshops | RSG | Done 2026-06-01
- [x] Email Laura to get refunded on Taki from RSG | RSG | Done 2026-06-01

## 🔴 Urgent (due this week) — NeoWorld

- [x] Outline June Lead Generator Calls | NeoWorld | Done 2026-06-09
- [x] Set Up ManyChat on NeoWorld Instagram | NeoWorld | Done 2026-06-09
- [x] Gamify Shipping Team with Bonuses | NeoWorld | Done 2026-06-09

## 🟡 This Cycle

- [ ] Ship Growth Plan v2 flow with Mark Brewer | RSG | Due Jun 30
- [ ] Film RSG VSL for YouTube | RSG | This cycle
- [x] Kick off Shipping Team Restructure | NeoWorld | Done 2026-06-09
- [ ] Build NWU QR One-Pager | NeoWorld | Waiting on Dillon
- [ ] Review Brand Deals with Tanner Milson | NeoWorld | This cycle
- [ ] Build Triangle Model for NeoWorld | NeoWorld | Due this week
- [x] Set up Basecamp workspace with Emily | Personal | Done 2026-06-01

## 🟢 Ongoing

- [ ] New Rule of Life (Practicing the Way) | Personal | Due Jun 26
- [x] Emotional honesty practice with Emily | Personal | Done 2026-06-09

---

## 📋 Basecamp — Assigned to Hunter

_Synced from Basecamp · Ready Set Grow · 2026-05-31_

### Momentum Staff HQ
- [x] Prep for Middle Method re-recording | Momentum Staff HQ | Done 2026-06-01
- [ ] SINGLE (omnibus) Assessment — covers all 3 areas | Momentum Staff HQ | Due Jun 26 | Waiting on Mark — where is he at with discovery assessment questions for the Growth Plan
- [ ] Rebuild the GP Google Doc toolkits and send to Mark and Emily | Momentum Staff HQ | Due Jun 17
- [ ] Update the GP cue sheet template in coaching call cue sheet | Momentum Staff HQ | Due Jun 17
- [ ] Film all tools in Loom for new Growth Plan | Momentum Staff HQ | Due Jun 15
- [ ] Create a Video Sales Letter | Momentum Staff HQ
- [x] A-Team Workshop | Momentum Staff HQ | Done 2026-06-09

### Content Tracking
- [x] Outline LIVE FROM TORREY PINES with Dr. Chand (30 min) | Content Tracking | Done 2026-06-01
- [x] Finish writing LIVE FROM TORREY PINES with Dr. Chand with assets (1 hr) | Content Tracking | Done 2026-06-01
- [ ] Edit Healthy Volunteer Culture and assets (45 min) | Content Tracking | Due Jul 1
- [ ] Outline Keeping First Time Guests (Engagement Pathway) (1 hr) | Content Tracking | Due Jul 8
- [ ] Send Keeping First Time Guests (Engagement Pathway) to team for editing | Content Tracking | Due Jul 28
- [ ] (Curriculum items & workshop items — 4 Notion-linked tasks) | Content Tracking

### Emily // Hunter
- [x] Build out the Emily // Hunter Basecamp project as a shared home base | Emily // Hunter | Done 2026-06-01
- [ ] Hunter: Rule of Life | Emily // Hunter | Due Jun 26
- [x] Proactive Emotional Honesty with Emily | Emily // Hunter | Done 2026-06-09
- [ ] 3rd Kid: Second Conversation | Emily // Hunter | Due Jun 26

#### Future Ideas (not this cycle)
- [ ] Hunter's Nutrition — address food habits | Emily // Hunter
- [ ] Hunter Slowing Down — build margin practices | Emily // Hunter
- [ ] Dillon + Holly Church Convo | Emily // Hunter
- [ ] Implement: The Intentional Father book | Emily // Hunter
- [ ] Friend Rhythms — Dillon+Holly, Meagan+Jordan, Tylor+Hannah | Emily // Hunter
- [ ] Date Nights — intentionally revive regular date nights | Emily // Hunter
- [ ] Shared Long-Term Future Document — create shared vision? | Emily // Hunter

---

_Last updated: 2026-06-09_
